...
def main():
    ...
    mesh_miner_contract, mesh_miner_address = deploy_contract(
        "mesh/MeshMiner", xmrt_address
    )

    # Set up roles (example addresses, replace with actual Eliza, Langflow, Validator addresses)
    eliza_address = (
        "0x742d35Cc6634C0532925a3b8D0C9e3e0C8b0e5B2"  # Example Eliza address
    )
    langflow_address = (
        "0x8ba1f109551bD432803012645Hac136c5c2BD754"  # Example Langflow address
    )
    validator_address = (
        "0x9f2d04a9c2f0f6c3e8b1a5d4c7e9f2a8b5c6d3e7"  # Example Validator address
    )

    print("\nSetting up roles...")
    # Grant CEO_AI_ROLE to Eliza
    ceo_ai_role = xmrt_contract.functions.CEO_AI_ROLE().call()
    tx = xmrt_contract.functions.grantRole(
        ceo_ai_role, eliza_address
    ).build_transaction({"nonce": w3.eth.get_transaction_count(account.address)})
    ...
    tx = xmrt_contract.functions.grantRole(
        audit_ai_role, langflow_address
    ).build_transaction({"nonce": w3.eth.get_transaction_count(account.address)})
    signed_tx = w3.eth.account.sign_transaction(tx, private_key=private_key)
    w3.eth.wait_for_transaction_receipt(
        w3.eth.send_raw_transaction(signed_tx.rawTransaction)
    ...
    tx = xmrt_contract.functions.grantRole(
        mesh_validator_role_xmrt, validator_address
    ).build_transaction({"nonce": w3.eth.get_transaction_count(account.address)})
    signed_tx = w3.eth.account.sign_transaction(tx, private_key=private_key)
    w3.eth.wait_for_transaction_receipt(
        w3.eth.send_raw_transaction(signed_tx.rawTransaction)
    ...
    mesh_validator_role_meshminer = (
        mesh_miner_contract.functions.MESH_VALIDATOR_ROLE().call()
    )
    tx = mesh_miner_contract.functions.grantRole(
        mesh_validator_role_meshminer, validator_address
    ).build_transaction({"nonce": w3.eth.get_transaction_count(account.address)})
    signed_tx = w3.eth.account.sign_transaction(tx, private_key=private_key)
    w3.eth.wait_for_transaction_receipt(
        w3.eth.send_raw_transaction(signed_tx.rawTransaction)
